...
def acc_score(x, y, nn_score = True):
    if nn_score:
        return accuracy_score(np.argmax(nn.functional.sigmoid(x), axis=1), y)
    else:
...
def evaluate(model, data, pred_types, training_data, new_ml_predictors=None):
    ...
    for pred_type in pred_types:
        if pred_type is None:
            ...
            loss_functions.append(lambda x, y: nn.CrossEntropyLoss()(x, y.to(torch.long)))
            score_functions.append(lambda x,y, nn_score: acc_score(x,y, nn_score))
    ...
    with torch.no_grad():
        ...
        for i, (score_fn, loss_fn) in enumerate(zip(score_functions, loss_functions)):
            if pred_types[i] is None:
                ...
                nn_score = score_fn(nn_predictions[i], ground_truth[:, i].unsqueeze(-1), nn_score=True)
                nn_scores.append(float(nn_score))
                ml_score = score_fn(ml_predictions[i], ground_truth[:, i].unsqueeze(-1), nn_score=False)
                ml_scores.append(ml_score)
                loss = loss_fn(nn_predictions[i], ground_truth[:, i])
                losses.append(loss)
