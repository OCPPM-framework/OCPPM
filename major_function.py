...
def main_function(filename, parameters, subg_funcs, g_funcs, no_feat, ks, embedding_name, embedding_size,
    ...
    for k in ks:
        ...
        pred_types = get_pred_types(subgraph_list, subg_funcs, g_funcs, task_names)
        ...
        _, test_ml_scores = evaluate(model, test_loader, pred_types, train_loader, new_ml_predictors=new_ml_preds)
        # Store scores
        for i, result_df in enumerate(result_dfs):
            if pred_types[i] is None:
        ...
        for pred_type in pred_types:
            if pred_type is None:
                target_dim = 1
                new_ml = MLPRegressor(hidden_layer_sizes=(32,32))
            else:
                target_dim = pred_type
                new_ml = MLPClassifier(hidden_layer_sizes=(32,32), solver='adam')
            new_ml_preds.append(new_ml)
        _, test_ml_scores = evaluate(model, test_loader, pred_types, train_loader, new_ml_predictors=new_ml_preds)
        # Store scores
        for i, result_df in enumerate(result_dfs):
